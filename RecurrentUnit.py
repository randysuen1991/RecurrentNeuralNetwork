...
class RecurrentUnit:
    def __init__(self, hidden_dim, input_dim, dtype=tf.float64):
        ...
        self.output = None
        self.input_transposed = None
        self.h_0 = None
        self.output_transposed = None
...
class VanillaRecurrentUnit(RecurrentUnit):
    ...
    def _forward_pass(self, output_m1, x_t):
        return tf.tanh(tf.matmul(x_t, self.parameters['wi']) +
                       tf.matmul(output_m1, self.parameters['wh']) + self.parameters['b'])
...
class GatedRecurrentUnit(RecurrentUnit):
    ...
    def initialize(self, input_dim):
        ...
        if self.full:
            ...
            self.parameters['wir'] = tf.Variable(initial_value=tf.truncated_normal(dtype=self.dtype,
                                                                                   shape=(self.input_dim,
                                                                                          self.hidden_dim),
                                                                                   mean=0, stddev=0.1))
            self.parameters['whr'] = tf.Variable(initial_value=tf.truncated_normal(dtype=self.dtype,
                                                                                   shape=(self.hidden_dim,
                                                                                          self.hidden_dim),
                                                                                   mean=0, stddev=0.1))
            self.parameters['br'] = tf.Variable(initial_value=tf.truncated_normal(dtype=self.dtype,
                                                                                  shape=(self.hidden_dim,),
                                                                                  mean=0, stddev=0.1))
            self.output_transposed = tf.scan(fn=self._forward_pass_full,
                                             elems=self.input_transposed, initializer=self.h_0)
        else:
    ...
    def _forward_pass_full(self, output_m1, x_t):
        f_t = tf.sigmoid(tf.matmul(x_t, self.parameters['wif']) +
                         tf.matmul(output_m1, self.parameters['whf']) + self.parameters['bf'])
        r_t = tf.sigmoid(tf.matmul(x_t, self.parameters['wir']) +
                         tf.matmul(output_m1, self.parameters['whr']) + self.parameters['br'])
        h_proposal = tf.tanh(tf.matmul(x_t, self.parameters['wio']) +
                             tf.matmul(tf.multiply(r_t, output_m1), self.parameters['who']) + self.parameters['bo'])
    ...
    def _forward_pass(self, output_m1, x_t):
        f_t = tf.sigmoid(tf.matmul(x_t, self.parameters['wif']) +
                         tf.matmul(output_m1, self.parameters['whf']) + self.parameters['bf'])
        h_proposal = tf.tanh(tf.matmul(x_t, self.parameters['wio']) +
                             tf.matmul(tf.multiply(f_t, output_m1), self.parameters['who']) + self.parameters['bo'])
...
class LongShortTermMemory(RecurrentUnit):
    def __init__(self, hidden_dim, input_dim=None, dtype=tf.float64):
        super().__init__(hidden_dim, input_dim, dtype)
        self.c_0_transposed = None
        self.c_t_transposed = None

    def initialize(self, input_dim):
        self.input_dim = input_dim
        # 'w' means weights, 'i' in the middle place means input data and 'f' means forget gate.
        self.parameters['wif'] = tf.Variable(initial_value=tf.truncated_normal(dtype=self.dtype,
                                                                               shape=(self.input_dim, self.hidden_dim),
                                                                               mean=0, stddev=0.1), name='Wif')
        # 'h' means the info from last time.
        self.parameters['whf'] = tf.Variable(initial_value=tf.truncated_normal(dtype=self.dtype,
                                                                               shape=(self.hidden_dim, self.hidden_dim),
                                                                               mean=0, stddev=0.1), name='Wif')
        # 'i' in the last place means input gate.
        self.parameters['wii'] = tf.Variable(initial_value=tf.truncated_normal(dtype=self.dtype,
                                                                               shape=(self.input_dim, self.hidden_dim),
                                                                               mean=0, stddev=0.1), name='Wif')
        self.parameters['whi'] = tf.Variable(initial_value=tf.truncated_normal(dtype=self.dtype, shape=(self.hidden_dim,
                                                                                                        self.hidden_dim),
                                                                               mean=0, stddev=0.1), name='Wif')
        ...
        self.parameters['wic'] = tf.Variable(initial_value=tf.truncated_normal(dtype=self.dtype,
                                                                               shape=(self.input_dim, self.hidden_dim),
                                                                               mean=0, stddev=0.1), name='Wif')
        self.parameters['whc'] = tf.Variable(initial_value=tf.truncated_normal(dtype=self.dtype,
                                                                               shape=(self.hidden_dim, self.hidden_dim),
                                                                               mean=0, stddev=0.1), name='Wif')
        ...
        self.parameters['bf'] = tf.Variable(initial_value=tf.truncated_normal(dtype=self.dtype,
                                                                              shape=(self.hidden_dim,),
                                                                              mean=0, stddev=0.1), name='Wif')
        self.parameters['bi'] = tf.Variable(initial_value=tf.truncated_normal(dtype=self.dtype,
                                                                              shape=(self.hidden_dim,),
                                                                              mean=0, stddev=0.1), name='Wif')
        self.parameters['bo'] = tf.Variable(initial_value=tf.truncated_normal(dtype=self.dtype,
                                                                              shape=(self.hidden_dim,),
                                                                              mean=0, stddev=0.1), name='Wif')
        self.parameters['bc'] = tf.Variable(initial_value=tf.truncated_normal(dtype=self.dtype,
                                                                              shape=(self.hidden_dim,),
                                                                              mean=0, stddev=0.1), name='Wif')

        self.input_transposed = tf.transpose(self.input, perm=[1, 0, 2])
        self.c_0_transposed = tf.matmul(self.input_transposed[0, :, :],
                                        tf.zeros(shape=(self.input_dim, self.hidden_dim), dtype=self.dtype))
        self.h_0 = tf.matmul(self.input_transposed[0, :, :], tf.zeros(shape=(self.input_dim, self.hidden_dim),
                                                                      dtype=self.dtype))
        self.output_transposed, self.c_t_transposed = tf.scan(fn=self._forward_pass,
                                                              elems=self.input_transposed,
                                                              initializer=(self.h_0, self.c_0_transposed))
    ...
    def _forward_pass(self, info_tm1, x_t):
        ...
        f_t = tf.sigmoid(tf.matmul(x_t, self.parameters['wif']) + tf.matmul(output_m1, self.parameters['whf']) +
                         self.parameters['bf'])
        i_t = tf.sigmoid(tf.matmul(x_t, self.parameters['wii']) + tf.matmul(output_m1, self.parameters['whi']) +
                         self.parameters['bi'])
        o_t = tf.sigmoid(tf.matmul(x_t, self.parameters['wio']) + tf.matmul(output_m1, self.parameters['who']) +
                         self.parameters['bo'])
        c_t = f_t * c_tm1 + i_t * tf.tanh(tf.matmul(x_t, self.parameters['wic']) +
                                          tf.matmul(output_m1, self.parameters['whc'] + self.parameters['bc']))
